...
{
    /// <summary>
    /// Pantalla de Asignación de Auditoría: permite asignar lotes pendientes de asignar
    /// auditoria (cdEstado=7) a un usuario auditor, cambiando su estado a "Auditando" (cdEstado=8)
    /// </summary>
    public partial class FrmAsignarAuditoria : Form
    {
        private const int CD_ESTADO_PENDIENTE_ASIGNAR_AUDITORIA = 7;
        ...
        public FrmAsignarAuditoria()
        {
            InitializeComponent();
        }

        private void FrmAsignarAuditoria_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void CargarLotes()
        {
            try
            {
                ...
                var lotes = loteDAL.ObtenerLotesPorEstadoFiltrado(
                    CD_ESTADO_PENDIENTE_ASIGNAR_AUDITORIA, dsNombreLote, feAltaDesde, feAltaHasta);
        ...
        private void btnAsignarLotes_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (cboUsuarios.SelectedValue is not int cdUsuarioAuditor)
                {
                ...
                var confirmacion = MessageBox.Show(
                    $"¿Confirma asignar {lotesSeleccionados.Count} lote(s) al usuario auditor seleccionado?",
                ...
                var loteDAL = new LoteDAL();
                loteDAL.AsignarAuditoria(lotesSeleccionados, cdUsuarioAuditor);
